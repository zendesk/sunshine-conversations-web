...
function immediateUpdate(props) {
    ...
    return isDirty ? (0, _core.core)().appUsers.update(user._id, props).then(function (response) {
...
function trackEvent(eventName, userProps) {
    var user = _appStore.store.getState().user;
    return (0, _core.core)().appUsers.trackEvent(user._id, eventName, userProps).then(function (response) {
...
function immediateUpdateDevice(deviceId, device) {
    return (0, _core.core)().appUsers.updateDevice(_appStore.store.getState().user._id, deviceId, device).then(function (response) {
        if (response.conversationUpdated) {
            return (0, _conversationService.handleConversationUpdated)().then(function () {
                return response;
            });
