    ...
    if (!client) {
        (function () {
            var _store$getState = _appStore.store.getState(),
                appState = _store$getState.appState,
            ...
            client.on('transport:up', function () {
                ...
                if (user.conversationStarted) {
                    (0, _conversationService.getMessages)();
                }
            });
        })();
    }
...
function subscribeConversation() {
    ...
    var _store$getState3 = _appStore.store.getState(),
        conversationId = _store$getState3.conversation._id;
...
function handleConversationActivitySubscription(_ref) {
    ...
    if (role === 'appMaker') {
        ...
        switch (activity) {
            case 'typing:start':
                return (0, _appService.showTypingIndicator)(data);
            case 'typing:stop':
                return (0, _appService.hideTypingIndicator)();
        }
    }
}
...
function subscribeConversationActivity() {
    ...
    var _store$getState4 = _appStore.store.getState(),
        conversationId = _store$getState4.conversation._id;

    var subscription = client.subscribe('/v1/conversations/' + conversationId + '/activity', handleConversationActivitySubscription);

    return subscription.then(function () {
        _appStore.store.dispatch((0, _fayeActions.setFayeConversationActivitySubscription)(subscription));
    });
}
...
function handleUserSubscription(_ref2) {
    ...
    var _store$getState5 = _appStore.store.getState(),
        currentAppUser = _store$getState5.user,
        visibleChannelType = _store$getState5.appState.visibleChannelType;
